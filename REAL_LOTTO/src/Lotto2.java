...
public class Lotto2 extends JPanel {
	...
	public RoundedButton moneybtn;
	public JLabel moneylbl;
...
//					System.out.println(Arrays.toString(Lotto_info.oneGame)); //oneGame에 잘 들어가는지 확인용
		...
		btnAuto.setFont(new Font("한컴 훈민정음 가로쓰기", Font.PLAIN, 18));
		...
		btnSAuto.setFont(new Font("한컴 훈민정음 가로쓰기", Font.PLAIN, 18));
		...
		btnEnter.setFont(new Font("한컴 훈민정음 가로쓰기", Font.PLAIN, 18));
		...
		JLabel helplbl = new JLabel("※ 도움말 보기");
...
//							goStop = false;
					...
					moneylbl.setText("소지금 : " + Lotto_info.money + "원");//돌아와서 돈필드 값 수정
		...
		helplbl.addMouseListener(new MouseAdapter() { // 도움말 누르면 dialog 뜸(소)
			@Override
