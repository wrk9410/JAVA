...
//		setSize(500,500);
		...
		JLabel lbl = new JLabel("등록완료된 게임목록");
		lbl.setFont(new Font("궁서체", Font.BOLD, 15));
		lbl.setBounds(147, 21, 155, 23);
		add(lbl);
		...
		JLabel lbl1 = new JLabel("게임 1");
		lbl1.setFont(new Font("궁서체", Font.PLAIN, 12));
		...
		JLabel lbl2 = new JLabel("게임 2");
		lbl2.setBounds(12, 23, 36, 15);
		lbl2.setFont(new Font("궁서체", Font.PLAIN, 12));
		pnls[1].add(lbl2);
		...
		JLabel lbl3 = new JLabel("게임 3");
		lbl3.setFont(new Font("궁서체", Font.PLAIN, 12));
		...
		JLabel lbl4 = new JLabel("게임 4");
		lbl4.setFont(new Font("궁서체", Font.PLAIN, 12));
		...
		JLabel lbl5 = new JLabel("게임 5");
		lbl5.setFont(new Font("궁서체", Font.PLAIN, 12));
		...
		RoundedButton btn = new RoundedButton("결과확인");
...
//				new Lotto3를 할 경우 닫히고 다시 열릴 경우 계속해서 새로운 class에 접근하여 값이 저장 안되어 인스턴스로 받아둠
//				결과창이 나올 경우 Lotto2,3 열린 창 사라지게 하기(
//				setVisible(false); // Lotto3끄기
				lt4.setVisible(true); // Lotto4열기 <-dialog한다음 madal하면 될듯
			}
		});
			...
			public void mouseReleased(MouseEvent e) {
				if (!Lotto2.editState) { // 이미 수정모드가 아닌 상태에서만 받을거임~
					...
					if (!Lotto_info.isArrEmpty(arr)) { // 비어있지 않아야 중괄호 안으로 들어갈거임
						...
						int result = 0;
						if (!Lotto_info.isArrEmpty(Lotto_info.oneGame)) { // regNum에 번호 들어있으면 (oneGame에 옮겨져있음)없어질 수도 있다는 안내창 소환
